...
function VibeRules() {
    return (
        <section className="bg-[#121214]/60 backdrop-blur-sm border border-gray-800 rounded-lg p-8 my-10 max-w-4xl mx-auto shadow-xl" aria-label="Server culture and rules">
            <h2 className="text-2xl font-bold text-[#cd41ff] mb-6 border-b border-gray-800 pb-3">{data.vibeRulesData.vibeRulesTitle}</h2>
            <ul className="space-y-4 mb-6">
                {data.vibeRulesData.rules.map((rule, index) => (
            ...
            <p className="text-gray-400 text-sm italic mt-6 border-t border-gray-800 pt-3">{data.vibeRulesData.vibeRulesNote}</p>
        </section>
    );
}
